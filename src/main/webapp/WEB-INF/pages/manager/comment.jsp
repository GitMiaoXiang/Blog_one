...
<%--弹出框--%>
<div class="am-modal am-modal-prompt" tabindex="-1" id="commentrepaly">
        ...
        <div class="tpl-portlet-components">
            <div class="portlet-title">
                <div class="caption font-green bold">
                    <span class="am-icon-code"></span> 新评论
                </div>
                    ...
                    <div class="am-panel-group" id="accordion">
                        <c:forEach items="${comment}" var="comment">
                            <div class="am-panel am-panel-default" style="margin-top: 15px;">
                                <div class="am-panel-hd" style="font-size: 13px;">
                                    <p class="am-panel-title" data-am-collapse="{parent: '#accordion', target: '#do-not-say-1'}">
                                        ...
                                        <c:if test="${comment.status=='1'}">
                                            ...
                                            ${comment.content}
                                        <span class="am-fr" style="padding-left: 50px; margin-top: 1px">${comment.formtime}</span>
                                        <span class="tpl-label-info am-fr js-modal-open am-btn am-btn-primary" style="margin-top: 1px" onclick="openmodelomment(${comment.userid},${comment.id})"> 回复
                                        <i class=" am-icon-share"></i>
                                    </span>
                                    </p>
                                </div>
                                <c:if test="${comment.customs==null}">
                                ...
                                <c:if test="${comment.customs!=null}">
                                    <div id="do-not-say-1" class="am-panel-collapse am-collapse am-in" style="font-size: 13px;">
                                        <c:forEach items="${comment.customs}" var="commentrepaly">
                                            <div class="am-panel-bd">
                                                    ${commentrepaly.content}
                                                <span class="am-fr" style="padding-left: 50px; margin-top: 1px">${commentrepaly.fortime}</span>
                                            </div>
                                        </c:forEach>
                                    </div>
                                </c:if>
...
<script type="text/javascript">
     ...
     * 弹出回复对话框
     */
    function openmodelomment(userid,id){
        $('#commentrepaly').modal({
            relatedTarget: this,
            onConfirm: function(e) {
                var msgrepalycontent="touserid="+userid+"&commentid="+id+"&content="+e.data+"";
                $.ajax({
                    url:'http://localhost:8081/managerblog/manager/comment/sendcomentmanager',
                    type:'post',
